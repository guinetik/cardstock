...
export async function loadBoardState(
  ...
  const [
    ...
    db
      .from("cards")
      .select(
        "id, external_id, title, status, epic, area, audience, assignee, assignee_id, raised_by, raised_on, shipped_on, needs, summary, summary_edited_at, body_md, body_edited_at, checklist_present, checklist_revision, checklist_edited_at, card_checklist_items(id, label, completed, position), lane_id, rank, priority, effort, planned_start_date, target_date, target_label, archived_at, archived_by, color, source_hash, source_text, updated_at, frontmatter_extra, card_tags(tag_id), card_links!card_links_from_card_fkey(to_card, kind)",
      )
      .eq("board_id", boardId),
