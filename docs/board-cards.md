...
Board cards rest as **`#id`, title, epic, and the decisions already made** — the two pen squares for priority and effort, plus a status word when it is not `backlog`. The filter bar can restrict by that same word: a Status menu lists whatever values this board actually has, each with its pen; pick one, or leave it on any. That is the bird's-eye scan. Unrated cards show nothing rather than empty slots. A selected `color` adds `card-color--{name}` on the `<article>` only (parsed with `parseCardColor`), tinting the paper; the drag overlay keeps the tint and omits the picker.
