...
- Heading **Comments**, `data-testid="comment-thread"` for parsed blocks (`at`, `author`, unquoted markdown via `marked`).
- Optional leftover (`data-testid="comment-leftover"`) for unparsed tail after `## Comments`.
- Composer (`data-testid="comment-composer"`) and **Post** (`data-testid="post-comment"`) call `addCardComment`. Empty input stays on the client (`Write a comment first.`). Success clears the composer and `router.refresh()`.
